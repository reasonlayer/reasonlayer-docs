...
Agents only see the hash value of the credential (often called `rlcred_...`, found after assigning the credential to the agent). To use the credential, either use the hash or the enviroment variable name.
...
The credential environment variable name you set in the Reasonlayer dashboard (e.g. `OPENROUTER_API_KEY`) is the same name your agent uses at runtime.
